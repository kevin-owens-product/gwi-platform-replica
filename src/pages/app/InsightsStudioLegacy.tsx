...
import { insightsStudioEnabled } from '@/features/insights/config'
...
export default function InsightsStudio(): React.JSX.Element {
  ...
  const [boardFilterOverrides, setBoardFilterOverrides] = useState<InsightFilterSet[]>([])
  const [storySteps, setStorySteps] = useState<InsightStoryStep[]>([])
  const [isStoryReviewMode, setIsStoryReviewMode] = useState(false)
  const [activeStoryStepId, setActiveStoryStepId] = useState<string | null>(null)
  ...
  const availableBoards = useMemo(
    () => assets.filter((asset) => asset.type === 'board'),
    [assets],
  )
  ...
  useEffect(() => {
    ...
    if (loadedAsset.board_config) {
      ...
      setBoardFilterOverrides(loadedAsset.board_config.filter_overrides ?? [])
    } else {
      setBoardViewIds([])
      setBoardFilterOverrides([])
    }

    if (loadedAsset.story_config) {
      setStorySteps(loadedAsset.story_config.steps)
      setActiveStoryStepId(loadedAsset.story_config.steps[0]?.id ?? null)
    } else {
      setStorySteps([])
      setActiveStoryStepId(null)
    }
  ...
  useEffect(() => {
    ...
    if (!activeStoryStepId || !storySteps.some((step) => step.id === activeStoryStepId)) {
      setActiveStoryStepId(storySteps[0].id)
    }
  }, [activeStoryStepId, storySteps])

  const executionContext = useMemo(() => resolveInsightExecutionContext({
    querySpec,
    viewMode,
    chartType,
    ...
    storySteps,
    activeStoryStepId: isStoryReviewMode ? activeStoryStepId : null,
    assets,
  }), [
    assets,
    boardFilterOverrides,
    chartType,
    insightDefaults.filterPresets,
    isStoryReviewMode,
    querySpec,
    storySteps,
    activeStoryStepId,
  ...
  const executeQuery = async (
    context: ReturnType<typeof resolveInsightExecutionContext>,
    ...
    try {
      ...
      trackInsightsTelemetry('insights.query.run', {
        ...
        review_mode: source === 'story_review',
  ...
  const handleRunQuery = async () => {
    await executeQuery(executionContext, 'manual')
  }

  const handleRunStoryStep = async (stepId: string) => {
    const stepContext = resolveInsightExecutionContext({
      querySpec,
      viewMode,
      chartType,
      workspaceFilterPresets: insightDefaults.filterPresets,
      boardFilterOverrides,
      storySteps,
      activeStoryStepId: stepId,
      assets,
    })

    setIsStoryReviewMode(true)
    setActiveStoryStepId(stepId)
    await executeQuery(stepContext, 'story_review')
  }
  ...
  const handleSaveBoard = async () => {
    const savedId = await saveAsset('board', {
      ...
      board_config: {
        ...
        filter_overrides: boardFilterOverrides,
      },
      tags: ['insights', 'board'],
    })
  ...
  const handleSaveStory = async () => {
    const savedId = await saveAsset('story', {
      ...
      story_config: {
        steps: storySteps,
        review_notes: isStoryReviewMode ? `Review mode active. Selected step: ${activeStoryStepId ?? 'none'}` : undefined,
      },
      tags: ['insights', 'story'],
    })
    ...
    <div className="insights-page">
      ...
      <div className="insights-grid">
        ...
        <InsightsBoardStoryPane
          ...
          boardFilterOverrides={boardFilterOverrides}
          onBoardFilterOverridesChange={setBoardFilterOverrides}
          storySteps={storySteps}
          onStoryStepsChange={setStorySteps}
          reviewMode={isStoryReviewMode}
          onReviewModeChange={setIsStoryReviewMode}
          activeStoryStepId={activeStoryStepId}
          onActiveStoryStepIdChange={setActiveStoryStepId}
          onRunStoryStep={handleRunStoryStep}
